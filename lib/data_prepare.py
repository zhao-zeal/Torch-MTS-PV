...
def get_dataloaders_scaled(
    ...
    """
    # 读取两个文件：原始时序数据、提前生成的窗口索引
    # 出偏差不会是因为 astype(np.float32) 吧
    data = np.load(os.path.join(data_dir, f"data.npz"))["data"]
    index = np.load(os.path.join(data_dir, f"index_{in_steps}_{out_steps}.npz"))
    ...
    len_train = train_index[-1][2]
    # 用训练段的第 0 通道计算 mean/std
    # 注意：这里必须用 float64，和 solar-energy 对齐
    train_target = data[:len_train, :, 0].astype(np.float64)

    mean = train_target.mean(axis=0)
    std = train_target.std(axis=0, ddof=0)
    std[std == 0] = 1.0

    scaler = StandardScaler(mean=mean, std=std)

    # 保存原始训练段，方便和 solar-energy 对比
    np.save("debug_torchmts_train_target_raw.npy", train_target.reshape(-1))
    np.savez("debug_torchmts_scaler_stats.npz", mean=mean, std=std)

    print("torchmts len_train:", len_train)
    print("torchmts train first 5:", train_target.reshape(-1)[:5])
    print("torchmts train last 5:", train_target.reshape(-1)[-5:])
    print("torchmts mean:", mean)
    print("torchmts std:", std)

    # 对全量目标通道做标准化
    data = data.astype(np.float64, copy=True)
    data[..., 0] = scaler.transform(data[..., 0].astype(np.float64))

    # 最后转回 float32，进入 TensorDataset / 模型训练
