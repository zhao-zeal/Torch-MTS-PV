...
def generate_data(
    ...
    elif split.upper() == "LTSF_SOLAR":
        # 与 solar-energy Dataset_Custom_Solar 对齐：
        # 先取前 70% 为 train_all，再从 train_all 中切 10% 做 val
        num_train_all = int(l * 0.7)
        num_vali = int(num_train_all * 0.1)
        split1 = num_train_all - num_vali   # 纯 train 结束位置
        split2 = num_train_all              # test 起点

        train_index = [
            (t - history_seq_len, t, t + future_seq_len)
            for t in range(history_seq_len, split1 - future_seq_len + 1)
        ]
        valid_index = [
            (t - history_seq_len, t, t + future_seq_len)
            for t in range(split1, split2 - future_seq_len + 1)
        ]
        test_index = [
            (t - history_seq_len, t, t + future_seq_len)
            for t in range(split2, l - future_seq_len + 1)
        ]
...
if __name__ == "__main__":
    ...
    param_dict["add_time_of_day"] = False # if add time_of_day feature
    param_dict["add_day_of_week"] = False # if add day_of_week feature
    ...
    if DATASET_NAME in ("METRLA", "PEMSBAY"):
        ...
        param_dict["valid_ratio"] = 0.2
        param_dict["split"] = "LTSF"
    elif DATASET_NAME in ("SKIPPD"):
        # 窗口长度，切分方式，有无时间特征
        param_dict["data_file_path"] = os.path.join("../data/", DATASET_NAME, f"{DATASET_NAME}.csv")
        param_dict["train_ratio"] = 0.63
        param_dict["valid_ratio"] = 0.07
        param_dict["date_format"] = "%Y/%m/%d %H:%M:%S"
        param_dict["split"] = "LTSF_SOLAR"
    else:
