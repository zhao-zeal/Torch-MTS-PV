...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=2026, help="Set random seed")
    ...
    seed_everything(args.seed) # 设置seed
    set_cpu_num(args.cpus) # 设置线程数

    GPU_ID = args.gpu_num
    os.environ["CUDA_VISIBLE_DEVICES"] = f"{GPU_ID}" # 限制程序只能看到指定的GPU
    ...
    model_class = model_select(model_name) # 为了集成多种模型，使用model_class
    ...
    model = model_class(**cfg["model_args"]).to(DEVICE)  # 实例化模型，并放到DEVICE上
    ...
    log = os.path.join(log_path, f"{model_name}-{dataset}-{args.seq_len}-{args.pred_len}-{now}.log")
    log = open(log, "a")
    log.seek(0) 
    log.truncate() 
    ...
    save = os.path.join(save_path, f"{model_name}-{dataset}-{args.seq_len}-{args.pred_len}-{now}.pt")

    # ---------------------- set loss, optimizer, scheduler ---------------------- #

    criterion = loss_select(cfg.get("loss", dataset))(**cfg.get("loss_args", {}))
    # Adam：Adaptive Moment Estimation 自适应矩估计 
    # 优化器根据梯度更新参数  决定梯度的方向和步长
    ...
    model = runner.train(
        ...
        early_stop=cfg.get("early_stop", 20),
